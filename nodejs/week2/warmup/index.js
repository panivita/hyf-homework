...
app.get("/numbers/add", (req, res) => {
  ...
  if (!first || !second) {
    return res.send(
      "First add numbers to url, e.g. /numbers/add?first=2&second=2"
    );
  }
  if (isNaN(first) || isNaN(second)) {
    return res.send("Both parameters should be numbers");
  } else {
...
app.get("/numbers/multiply/*", (req, res) => {
  return res.send("First add numbers to url, e.g. /numbers/multiply/2/2");
});
